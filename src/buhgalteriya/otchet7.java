...
public class otchet7 extends javax.swing.JFrame {
    public otchet7() {
        initComponents();
    }
    ...
    private void initComponents() {
        ...
        setTitle("На яку категорію найменше витрачено грошей");
        setFocusable(false);
        setResizable(false);
...
"dokhody r \n" +
"WHERE s.ID=(SELECT ID_category FROM podkategorii_dokhodov WHERE ID_subcategory=r.podkategoriy_dokhoda) AND Year(date)='"+rik+"' AND Month(Date)='"+Number+"' AND Day(Date)>=1 AND Day(Date)<=31 GROUP BY s.NameCategory ORDER BY r.summa ASC LIMIT 1";
           stmt = con.createStatement();
            ResultSet rs = stmt.executeQuery(query);jTable1.setModel(DbUtils.resultSetToTableModel(rs));
