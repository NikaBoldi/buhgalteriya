...
public class otchet5 extends javax.swing.JFrame {public otchet5() {
        initComponents();
    }
@SuppressWarnings("unchecked")
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        ...
        try {
            ...
            String query ="SELECT  DISTINCT(Date) AS 'Дата', nameschet AS 'Назва рахунку', namesub AS 'Підкатегорія',cnt AS'Кількість', units AS 'Одиниця виміру', summa as 'Сума', primechanie AS 'Примітка' FROM raskhody,\n" +
"(SELECT Name AS nameschet FROM (SELECT schet AS a, podkategoriy_raskhodov AS b, summa as c, primechanie AS d FROM raskhody) AS quer1, schet WHERE ID=a) AS tab1, \n" +
"(SELECT NameSubcategory AS namesub FROM (SELECT schet AS a, podkategoriy_raskhodov AS b, summa as c, primechanie AS d FROM raskhody) AS quer2, podkategorii_raskhodov WHERE ID_subcategory=b) AS tab2\n" +
"WHERE Year(Date)='"+rik+"' AND Day(Date)>=01 AND Day(Date)<=31 AND Month(Date)='"+Number+"' GROUP BY Date";
            //String query ="SELECT * FROM schet";
